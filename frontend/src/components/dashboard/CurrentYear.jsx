...
const CurrentYear = () => {
  ...
  const fetchYearlySummary = async () => {
    try {
      ...
      const response = await expensesAPI.getYearlySummary(userId);
      
      console.log('Yearly expenses response:', response);
      ...
      if (Array.isArray(response)) {
        ...
        transformedData = response.data.categoryBreakdown.map(cat => ({
          ...
          products: [] // Yearly summary doesn't have product details
        }));
      ...
      setError('Failed to load yearly summary: ' + err.message);
      setExpenseData([]);
    } finally {
      setLoading(false);
    }
