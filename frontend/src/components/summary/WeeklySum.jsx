...
const WeeklySum = () => {
  ...
  useEffect(() => {
    fetchWeeklySummary(currentWeek);
  }, [currentWeek]);
  ...
  const fetchWeeklySummary = async (weekStart) => {
    try {
      ...
      // Sample data
      const sampleData = [
        { category: 'Groceries', amount: 275.50, color: '#10B981' },
        { category: 'Transport', amount: 89.25, color: '#EF4444' },
        { category: 'Food & Dining', amount: 165.75, color: '#F59E0B' },
        { category: 'Entertainment', amount: 120.00, color: '#8B5CF6' },
        { category: 'Utilities', amount: 85.30, color: '#3B82F6' },
        { category: 'Shopping', amount: 234.80, color: '#EC4899' },
        { category: 'Healthcare', amount: 45.90, color: '#14B8A6' }
      ];
      ...
      <div ref={summaryRef} className="max-w-4xl mx-auto space-y-6">
        ...
        {loading && (
          <div className="bg-white rounded-xl shadow-lg p-6 text-center">
            <div className="flex items-center justify-center space-x-2">
              ...
              <span className="text-gray-600">Loading weekly summary...</span>
            </div>
          </div>
        )}
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-800">
                    Rs. {(totalSpent / 7).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </p>
                </div>
        ...
        {!loading && expenseData.length === 0 && (
          <div className="bg-white rounded-xl shadow-lg p-12 text-center">
            ...
            <p className="text-gray-500">No expenses recorded for week of {formatWeekRange(currentWeek)}</p>
          </div>
        )}
